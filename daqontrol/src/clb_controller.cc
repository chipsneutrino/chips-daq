...
bool CLBController::setInitValues()
{
    ...
    var_ids.push_back(ProcVar::OPT_HR_VETO_THRES);      var_values.push_back(config_.veto_value_);  // HR Veto Value

    if(config_.veto_enabled_) // Enable HR Veto (all Channels)  
    {
        var_ids.push_back(ProcVar::OPT_HR_VETO_ENA_CH);     
        var_values.push_back(0xFFFFFFFF); 
    } 
    else // Disable HR Veto (all Channels)   
    {
        var_ids.push_back(ProcVar::OPT_HR_VETO_ENA_CH);     
        var_values.push_back(0x00000000);  
    }
