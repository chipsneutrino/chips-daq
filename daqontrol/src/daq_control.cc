...
void DAQControl::handleConfigCommand(std::string config_file)
{
    ...
    for (int c = 0; c < config_.num_controllers_; c++) {
        if (config_.configs_[c].type_ == CLB)
            controllers_.push_back(new CLBController(config_.configs_[c]));
        else if (config_.configs_[c].type_ == BBB)
            controllers_.push_back(new BBBController(config_.configs_[c]));
    }
...
{
    log(INFO, "DAQControl: Starting Run of type {}", (int)which);
    if (config_.is_nano_enabled_ && which != RunType::TestFlasher) {
        log(ERROR, "A flasher is enabled! Incorrect run type!");
    } else if (!config_.is_nano_enabled_ && which == RunType::TestFlasher) {
        log(ERROR, "No flashers are enabled! Needed for this run type!");
    }
}

void DAQControl::handleStopRunCommand()
{
    log(INFO, "DAQControl: Stopping Run");
}
...
void DAQControl::runAsync()
{
    ...
    log(INFO, "DAQ Control starting I/O service on {} threads", 1);
    for (int i = 0; i < 1; ++i) {
...
void DAQControl::stateUpdate()
{
    ...
    for (auto c : controllers_) {
        if (!c->dropped() && c->config_.enabled_) {
            ...
            if (state != target_state_ && !c->isWorking()) {
                log(WARNING, "Dropping controller ({})", c->getID());
    ...
    if (states.size() != 0) {
        if (std::equal(states.begin() + 1, states.end(), states.begin()) && states[0] != current_state_) {
            ...
            log(INFO, "DAQControl State Change to: {}", (int)current_state_);
        }
    }
