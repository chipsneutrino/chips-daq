...
int main(int argc, char* argv[]) {
	...
	desc.add_options()
        ...
        ("clb_frac", boost::program_options::value<float>(&clb_frac),
          		    "Fraction of CLB packets to use (1.0)")
		("bbb_frac", boost::program_options::value<float>(&bbb_frac),
	...
	if (save_elastic) { setup += "elastic"; }
	if (save_file) { setup += ",file"; }
	if (use_gui) { setup += ",gui"; }
