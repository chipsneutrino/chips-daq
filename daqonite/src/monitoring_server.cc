...
void MonitoringServer::handleCLBSocket(boost::system::error_code const& error, std::size_t size)
{
    if (!error) {
        ...
        const SCData* const scData = static_cast<const SCData* const>(static_cast<const void* const>(&fCLB_buffer[0]
            + clb_minimum_size));
        ...
        std::string message = "";
        if (fSave_elastic) {
            g_elastic.monitoringPacket(fCLB_run_num, fCLB_pom_id, fCLB_timestamp,
