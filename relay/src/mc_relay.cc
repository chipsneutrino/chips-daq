...
MCRelay::MCRelay(int ip)
    : Relay(ip, PORT_8020)
{
    setUnitName("MCRelay[{}]", ip_);
    log(INFO, "Created MCRelay({})", ip_); 
}
...
void MCRelay::on(int channel)
{
    ...
    if (n < 0) log(ERROR, "MCRelay ({}) could not send on command!", ip_); 

    usleep(DELAY_8020);

    char buffer[256];
    n = read(fd_, buffer, 255);
    if (n < 0) log(ERROR, "MCRelay ({}) could not read on response!", ip_); 
}
...
void MCRelay::pulse(int channel)
{
	...
	int n = send(fd_, msg, sizeof(msg), 0);
    if (n < 0) log(ERROR, "MCRelay ({}) could not send pulse command!", ip_); 

    usleep(DELAY_8020);

    char buffer[256];
    n = read(fd_, buffer, 255);
    if (n < 0) log(ERROR, "MCRelay ({}) could not read pulse response!", ip_); 
}
...
void MCRelay::off(int channel)
{
	...
	int n = send(fd_, msg, sizeof(msg), 0);
    if (n < 0) log(ERROR, "MCRelay ({}) could not send off command!", ip_); 

    usleep(DELAY_8020);

    char buffer[256];
    n = read(fd_, buffer, 255);
    if (n < 0) log(ERROR, "MCRelay ({}) could not read off response!", ip_);
}
...
void MCRelay::status()
{
	...
	int n = send(fd_, msg, sizeof(msg), 0);
    if (n < 0) log(ERROR, "MCRelay ({}) could not send status command!", ip_);  

    usleep(DELAY_8020);

    char buffer[256];
    n = read(fd_, buffer, 255);
    if (n < 0) log(ERROR, "MCRelay ({}) could not read status response!", ip_); 
