 ...
 *
 * Implements a bare-bones data access server which serves
 * buffer pages over a messaging interface.
 */
...
_handle_poll_page(void *ctx, fh_message_t *msgin, fh_transport_t *transport)
{
    ...
    bool page_available = (*(self->pagebuf.pop_page))(self->pagebuf.ctx, &(page_sink){.ctx=msgin, .callback=&_copy_page_to_msg});
    if (page_available) {
